    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (!itemstack.isEmpty() && itemstack.getItem() == Items.DYE) {
            ...
            if (enumdyecolor.getMetadata() == itemstack.getItem().getMetadata(15)) {
                ...
                worldIn.setBlockState(pos, ModBlocks.grass_stairs.getDefaultState().withProperty(FACING, blockstate.getValue(FACING)).withProperty(SHAPE, blockstate.getValue(SHAPE)).withProperty(HALF, blockstate.getValue(HALF)), 3);
