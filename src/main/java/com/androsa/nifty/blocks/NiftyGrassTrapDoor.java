...
public class NiftyGrassTrapDoor extends NiftyTrapDoor {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (!itemstack.isEmpty() && itemstack.getItem() instanceof ItemSpade) {
            ...
            worldIn.setBlockState(pos, ModBlocks.dirt_trapdoor.getDefaultState().withProperty(FACING, blockstate.getValue(FACING)).withProperty(OPEN, blockstate.getValue(OPEN)).withProperty(HALF, blockstate.getValue(HALF)), 3);
