...
public class NiftyIceSlab extends NiftySlab {

    private final float sliperiness;
    private final boolean canMelt;

    public NiftyIceSlab(NiftyBlock block, float slip, boolean melt) {
        super(block);
    ...
    public boolean isSideInvisible(BlockState state, BlockState otherState, Direction direction) {
        if (canMelt) {
            if (otherState.getBlock() == this && otherState.get(TYPE) == SlabType.DOUBLE && state.get(TYPE) == SlabType.DOUBLE) {
                return true;
            }
        }
